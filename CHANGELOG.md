...
- 移除旧版 Web4 Browser 宣传式图片和单独宣传段落；后续只在对应 FAQ 中加入带结论边界的中性技术工作流图，产品事实继续使用与其他产品相同的字段、来源和证据规则。

### README 视觉资产更新

- 将 README 恢复为四图结构：选择指南封面、12 款核心产品公开差异、硬条件选型图和 Web4 Browser AI 原生工作流。
- 删除仍以 6 款产品为范围的旧差异图和旧选型图，避免图片与 12 款核心数据互相冲突。
- 所有新版图片使用精确文字排版、1920×1080 横版尺寸，并同时保留 PNG 与 WebP。
- Web4 Browser 工作流图改用通用浏览器线框图标，不绘制或仿造 Chrome、Firefox 等第三方品牌 Logo。
- 将引用版本更新为 `1.2.1`，对应本次 README 视觉资产修复。
